...
CREATE PROC [MERGEPROCESS_New].[MergeDetect_MSCRM] --'jaguars'
    @Client VARCHAR(100) 
AS
Declare @Date Date = (select cast(getdate() as date));
DECLARE @Account VARCHAR(100) = (SELECT CASE WHEN @client = 'jaguars' THEN 'CRM_Accounts' ELSE CONCAT(@client,' PC_SFDC Account' ) END);--updateme
DECLARE @Contact VARCHAR(100) = (SELECT CASE WHEN @client = 'jaguars' THEN 'CRM_Contacts' ELSE CONCAT(@client,' PC_SFDC Account' ) END);--updateme
