...
INTO #WrongCRMID
FROM dbo.contact c
INNER JOIN jaguars.mdm.PrimaryFlagRanking_Contact r
ON c.SSB_CRMSYSTEM_CONTACT_ID = r.SSB_CRMSYSTEM_CONTACT_ID AND r.SourceSystem = 'CRM_Contacts' AND r.ss_ranking = 1
...
FROM dbo.contact a
INNER JOIN jaguars.mdm.PrimaryFlagRanking_Contact r
ON a.SSB_CRMSYSTEM_CONTACT_ID = r.SSB_CRMSYSTEM_CONTACT_ID AND r.SourceSystem = 'CRM_Contacts' AND r.ss_ranking = 1
...
WHERE b.SourceSystem = 'CRM_Contacts' AND a.[crm_id] != b.ssid --updateme
 AND c.[crm_id] IS NULL ;
GO
