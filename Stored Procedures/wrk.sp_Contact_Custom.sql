...
-- Purpose: CRM Custom Field Value Logic
...
-- 6/28/2018: Jeff Barberio

	-- Change notes: ALTERed season logic to account for new syntax in 2018 
	
	-- Peer reviewed by: Jeff Barberio
	
	-- Peer review notes: 
	
	-- Peer review date: 6/29/2018
...
FROM (
						 ...
						 from Jaguars.dbo.factticketsales fts (NOLOCK)
			 				 JOIN  (select DISTINCT fts.dimseasonid, dimcustomerid
			 						from Jaguars.dbo.factticketsales fts (NOLOCK)
			 							 JOIN Jaguars.dbo.dimseason ds (NOLOCK) on ds.dimseasonid = fts.dimseasonid
			 						 where dimeventid = 0
										  AND (ds.seasonname = concat(ds.seasonyear,' Jaguars Season Tickets')
												OR ds.seasonname = concat(ds.seasonyear,' Jaguars Manifest')
												OR (ds.SeasonName LIKE '%playoffs%' AND ds.SeasonName NOT LIKE '%parking%')
												OR ds.seasonname = concat(ds.seasonyear,' Jaguars')
												)
		...
		WHERE dc.SourceSystem = 'tm'
			  ...
			  AND (dimseason.seasonname = concat(dimseason.seasonyear,' Jaguars Season Tickets')
					OR dimseason.seasonname = concat(dimseason.seasonyear,' Jaguars Manifest')
					OR (dimseason.SeasonName LIKE '%playoffs%' AND dimseason.SeasonName NOT LIKE '%parking%')
					OR dimseason.seasonname = concat(dimseason.seasonyear,' Jaguars')
					)
	 )x
