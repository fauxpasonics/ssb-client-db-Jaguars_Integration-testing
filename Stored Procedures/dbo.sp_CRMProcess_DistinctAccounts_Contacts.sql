...
AS
/*
SELECT COUNT(*) FROM dbo.CRMProcess_DistinctAccounts
--537734
SELECT COUNT(*) FROM dbo.CRMProcess_DistinctContacts
--545206
*/
--SELECT * 
...
Select Distinct c.SSB_CRMSYSTEM_ACCT_ID, c.SSB_CRMSYSTEM_CONTACT_ID, c.DimCustomerId, c.[SourceSystem], --d.[MaxTransDate] 
'1/1/1900' MaxTransDate
from #tmpTrans a 
inner join dbo.vwDimCustomer_ModAcctId b on a.DimCustomerId = b.DimCustomerId and b.CustomerType = 'Primary'
inner join dbo.vwDimCustomer_ModAcctId c on b.AccountId = c.AccountId
--INNER JOIN dbo.[vwCRMProcess_CRMQualify_Transactions] d ON [d].[DimCustomerId] = [c].[DimCustomerId]
Where c.CustomerType <> 'Primary'

-- Bring in StdLeads Records
--INSERT INTO #tmpTrans
--Select Distinct b.SSB_CRMSYSTEM_ACCT_ID, b.SSB_CRMSYSTEM_CONTACT_ID, b.DimCustomerId, b.[SourceSystem], --d.[MaxTransDate] 
--'1/1/1900' MaxTransDate
--FROM dbo.vwDimCustomer_ModAcctId b 
--WHERE b.SourceSystem LIKE 'StdLead%' 

--DROP TABLE #tmpStdLeadsRecords
...
--SELECT COUNT(*) FROM [dbo].[vwCRMProcess_DistinctAccounts_CriteriaMet]
--SELECT COUNT(*) FROM [dbo].[vwCRMProcess_DistinctContacts_CriteriaMet]
