...
CREATE PROCEDURE [wrk].[sp_Contact_Custom_QA]
AS 

MERGE INTO dbo.contact_custom_DEV Target
...
, emailaddress2 = dc.EmailOne
, emailaddress3 = dc.EmailTwo
FROM dbo.contact_custom_DEV INNER JOIN dbo.vwCompositeRecord_ModAcctID dc 
ON dc.SSB_CRMSYSTEM_CONTACT_ID = contact_custom_DEV.SSB_CRMSYSTEM_CONTACT_ID
...
UPDATE c
SET  New_Turnkey_Age				 = substring(ta.AgeInTwoYearIncrements_InputIndividual,5,2)
...
FROM dbo.contact_custom_DEV c
	JOIN (
			SELECT SSB_CRMSYSTEM_CONTACT_ID
				  ,SUM(CASE WHEN SeasonYear = 2017 THEN QTY END)																	AS NumTickets_CY                     
				  ,SUM(CASE WHEN SeasonYear = 2017 THEN NumEvents END)																AS NumEvents_CY                 
				  ,1.0*SUM(CASE WHEN SeasonYear = 2017 THEN QTY END)/NULLIF(SUM(CASE WHEN SeasonYear = 2017 THEN NumEvents END),0)	AS TicketsPerEvent_CY                 
				  ,SUM(CASE WHEN SeasonYear = 2017 THEN REV END)																	AS TotalSpend_CY                 
				  ,1.0*SUM(CASE WHEN SeasonYear = 2017 THEN REV END)/NULLIF(SUM(CASE WHEN SeasonYear = 2017 THEN NumEvents END),0)	AS AvgSpend_CY
				  ,1.0*SUM(CASE WHEN SeasonYear = 2017 THEN REV END)/NULLIF(SUM(CASE WHEN SeasonYear = 2017 THEN QTY END),0)		AS AvgPrice_CY
				  ,SUM(CASE WHEN SeasonYear = 2016 THEN QTY END)																	AS NumTickets_PY                     
				  ,SUM(CASE WHEN SeasonYear = 2016 THEN NumEvents END)																AS NumEvents_PY                 
				  ,1.0*SUM(CASE WHEN SeasonYear = 2016 THEN QTY END)/NULLIF(SUM(CASE WHEN SeasonYear = 2016 THEN NumEvents END),0)	AS TicketsPerEvent_PY                 
				  ,SUM(CASE WHEN SeasonYear = 2016 THEN REV END)																	AS TotalSpend_PY                 
				  ,1.0*SUM(CASE WHEN SeasonYear = 2016 THEN REV END)/NULLIF(SUM(CASE WHEN SeasonYear = 2016 THEN NumEvents END),0)	AS AvgSpend_PY
				  ,1.0*SUM(CASE WHEN SeasonYear = 2016 THEN REV END)/NULLIF(SUM(CASE WHEN SeasonYear = 2016 THEN QTY END),0)		AS AvgPrice_PY
